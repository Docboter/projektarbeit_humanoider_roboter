...
| [cli-menuefuehrung.md](cli-menuefuehrung.md) | **Geführte CLI-Menüs für Training und Simulation — Plan, noch nichts umgesetzt.** Idee vom 2026-08-20: Startet man ein Skript ohne Parameter, fragt es die nötigen Werte ab und erklärt sie. Kern des Plans ist nicht das Menü selbst, sondern eine **deklarative Parameter-Spezifikation als einzige Quelle**, aus der Menü, `usage()` und die Doku-Tabellen erzeugt werden — sonst wäre das Menü die fünfte Kopie derselben Beschreibungen (heute: 99 Env-Vars und ein 200-Zeilen-`usage()` allein in [`server_rl_run.sh`](../../Simulation/server_rl_run.sh)). Reines Bash, nur auf dem Host, Container bleibt autonom; 7 Phasen mit Abnahmekriterien, Testplan, Risiken. |
