...
| **4 — Spur A testen** | `livecheck`, dann `./Simulation/server_rl_run.sh view` (Szene ohne Modell/Checkpoint/`HF_TOKEN` — seit 2026-08-17 der Einstieg mit den wenigsten beweglichen Teilen), erst danach `NUM_EPISODES=2 EPISODE_LENGTH_S=120 LIVESTREAM=2 … eval`; nativer WebRTC-Client → Viewport sichtbar und flüssig? | ~2 h | ⬜ **offen — der nächste Schritt.** `view` trennt dabei die Fehlerquellen: scheitert schon er, liegt es an Isaac Sim/GPU/Ports, nicht am Modell. Bei Fehlschlag: Isaac-Sim-6.0-Web-Viewer (Port 8210, Docker Compose) als zweiter Versuch; sonst bleibt Spur B die Lösung |
