...
| [synth-datensatz.md](synth-datensatz.md) | **Fremder Sim-Datensatz im Co-Training** (`Fichtl00/Cube_Stacking_synth`, 16 Episoden teleoperierter G1). Er passt schematisch nicht (57-Dim-State statt 28, 50 statt 30 fps, andere Schlüsselnamen); [`harmonize_synth_dataset.py`](../../Training/scripts/harmonize_synth_dataset.py) schreibt ihn um und **misst** die Gelenk-Zuordnung, statt sie zu raten. Begründetes Mischungsverhältnis (0,10 statt 0,25), Split nur auf den echten Episoden, vorregistrierte Bewertungsregel. Gelenkreihenfolge aus der Isaac-Tabelle des eigenen USD-Assets aufgelöst und gegengeprüft. **Werkzeug abgenommen, Lauf steht aus.** |
