...
Diese Datei ist deshalb eine Kopie von N1.7s ``gr00t/experiment/launch_finetune.py``
(Upstream-Commit 376ba89) mit exakt einer inhaltlichen Abweichung — der Datensatz-Liste
wird um den gerenderten Datensatz ergänzt — und lebt bewusst HIER statt im Submodul: eine
Änderung an ``app/Groot-1.7`` hieße Fork pushen, Dockerfile-Pin nachziehen, Image neu
...
Beim Anheben der GR00T-N1.7-Version diese Datei gegen das Original gegenprüfen:
alles außer dem ``datasets``-Block soll identisch bleiben (vgl. launch_cotrain.py für
den N1.6-Fall).

N1.7-Spezifika, die aus launch_finetune.py übernommen wurden:
  • EmbodimentTag.resolve() statt direkter Enum-Konstruktion
  • model_name = "nvidia/Cosmos-Reason2-2B" (statt Eagle-Backbone)
  • kein eagle_collator-Flag mehr
  • zusätzliche FinetuneConfig-Felder: use_percentiles, shortest_image_edge/crop_fraction,
    ds_weights_alpha, save_only_model, resume_from_checkpoint, skip_weight_loading
  • dataset_path wird os.pathsep-getrennt in eine Liste zerlegt — für Co-Training bleibt
    der ECHTE Datensatz in ft_config.dataset_path (kann selbst schon mehrere Pfade
    enthalten), der GERENDERTE kommt separat über --cotrain_dataset_path.

Aufruf (macht run_finetuning_cotrain.sh bei GROOT_VERSION=1.7):
    python launch_cotrain_n17.py --cotrain_dataset_path /data/cotrain/g1_dex3_rendered \
...
if __name__ == "__main__":
    ...
    # Echter Datensatz: os.pathsep-getrennte Liste wie im Original launch_finetune.py.
    ...
    # ── Ab hier Zeile für Zeile wie launch_finetune.py (N1.7) ─────────────────
